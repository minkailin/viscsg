...
pro multiplot,help=help,pmulti $
  ...
  colspacing=colspacing,rowspacing=rowspacing,margins=marginsin, $
...
common multiplot $
  ...
  ,colmargin   $; Margin between multiplotted columns
  ,rowmargin    ; Margin between multiplotted rows
;
;	=====>> HELP
;
;print,"================= multiplot ================"
...
if init or keyword_set(initialize) then begin
  ...
  if not(keyword_set(colspacing)) then colmargin=0 else colmargin = colspacing
  if not(keyword_set(rowspacing)) then rowmargin=0 else rowmargin = rowspacing
  !p.position = 0				; reset
  !p.multi = 0
  plot,/nodata,xstyle=4,ystyle=4,!x.range,!y.range,/noerase	; set window & region

  if not(keyword_set(marginsin)) then $
  margins = [min(!x.window)-min(!x.region) $	; in normlized coordinates
	...
	else margins=marginsin
	if keyword_set(extramargins) then margins += extramargins
  noerase = !p.noerase
...
rowheight = (1.-margins[1]-margins[3]) / (rows + (rows-1)*rowmargin)
colwidth = (1.-margins[0]-margins[2]) / (cols + (cols-1)*colmargin)
...
if colmajor then begin			; location in matrix of plots
  ...
  col = (cur-1)/rows + 1
endelse
pos = [(col-1)*colwidth*(1+colmargin),$
	   (rows-row)*rowheight*(1+rowmargin),$
	   (col-1)*colwidth*(1+colmargin)+colwidth,$
	   (rows-row)*rowheight*(1+rowmargin)+rowheight ]$
	   + [margins[0], margins[1], margins[0], margins[1]]  ; this line adds offset for bottom left corner based on margins
;print,row,col,rows,cols,pos
if total(pos gt 1) gt 0 then message,"Invalid position vector calculated?! pos >1.0!"
;
