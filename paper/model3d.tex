...
{\bf For simplicity we refer to a single adiabatic index as that
  appears in the equation of state for both 2D and 3D disk models 
...
{\bf Here, $T_\mathrm{irr}$}
%Here, 
%\begin{align}\label{tirr_def}
%  \tirr = \theta T_\mathrm{eq}
%\end{align}
is a reference temperature field. 
%parameterized as a 
%fraction $\theta$ of the equilibrium temperature field 
%$T_\mathrm{eq}(z)$. 
Physically, $\tirr$ {\bf may} the floor temperature set by, 
for example, stellar or background irradiation. {\bf 
...
\rho(z=0)$ is the equilibrium mid-plane density. {\bf Note that
  $\Gamma$ is only relevant 
  to the 3D problem as it determines the disk vertical structure.} 
