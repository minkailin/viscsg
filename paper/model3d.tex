...
to bring out the physical process of interest --- viscous GI. %expect
                                %to 
%be generally present, but will 
...
%Furthermore, \cite{paardekooper12} measured
...
the `irradiation level'. %stellar illum or env rad      
...
\end{align}
%This relation applies if 
%For generality, we will formulate the linear problem without assuming
%Eq. \ref{alpha_beta_relation} (i.e. treat $\alpha$ and $\beta$ as
%independent) and only apply it in numerical
%calculations.   

%{\bf prob formulation treats alpha-beta independent. this relation
%  only needed if hext is zero}
