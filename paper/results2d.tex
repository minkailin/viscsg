...
We use this generalized dispersion relation, which does not assume any
$\alpha$-$\beta$ relation, %dispersion relation treats alpha/beta as
                           %independent 
to investigate GI driven
...
\end{align}
are both satisfied. If only real growth rates are considered, then the
second condition alone is sufficient for stability. %from property of
                                %cubic crossing the real axis 
...
depend on the actual cooling time $\beta$ (provided it is non-zero and
finite). The system becomes more unstable with decreasing 
irradiation levels $\theta$, i.e. as the floor temperature, towards which
perturbations are cooled, drops. 
...
for finite $Q$: instability is always possible at sufficiently small
scales (recall the discussion in \S\ref{cool_gi}). At fixed $Q$,  
the most unstable wavelength decreases with
