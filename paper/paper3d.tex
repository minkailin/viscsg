...
%qualitative intro to gravito-turbulence 
%role of cooling and resulting viscosity 
...
dispersion relation, are still widely applied to accretion disk models
...
GI in realistic accretion disk models. Specifically, we will examine how the
effects of cooling and viscosity can lead to GI by removing pressure
and rotational support, respectively, even when the Toomre condition
for instability is not satisfied. 
...
possible for any finite $Q$. This shows that cooling can change the
qualitative nature of GI compared to the classic Toomre condition. 

%Eq. \ref{cool_dr} is a cubic in $s$ and differs qualitatively from the
%classic result, Eq. \ref{classic_dr}, which is quadratic in $s$. In
%fact, 
...
Viscous disks can develop GI even when $Q>1$. This is because, as demonstrated below, viscosity compromises rotational support against
...
%\begin{align}
$  2\pi G \Sigma > c_s^2|k|$, is identical that obtained from 
Eq. \ref{classic_dr} with 
%\end{align}
...
% Then the disk temperature will be lowered 
% until $Q\sim 1$, whence non-axisymmetric modes grow 
% and heats the disk through dissipation of spiral shocks. This may 
% lead to a quasi-steady turbulent state with $Q\sim 1$ in
% which cooling is balanced by shock heating to maintain thermal
% equilibrium \citep{gammie01}.  
